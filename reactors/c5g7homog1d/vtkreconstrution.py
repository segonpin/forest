...
def reconstruct_data(fun_name="Phi",rootf="input"):
  ...
  def is_ass_file(f):
      return(f.endswith((".vtk")) and rootf+"Ass" in f and not "_geom" in f)
  mypath = "."
  ...
  for i in range(n_fun):
    fun[i] = np.concatenate([norm2(all_x,all_array,ass,j) \
